...
def escape(src):
    """
    Escape colon characters from feature names: `:` is used as part of feature names, so it must be escaped.
    :type   src:    str
...
def item_sequence(X):
    """
    Convert an item sequence into an object compatible with pycrfsuite
    Python module.

    Parameters
    ----------
    X: list of mapping objects
        The sequence features.

    :rtype         pycrfsuite.ItemSequence
    :return        The same sequence in crfsuite.ItemSequence type.
    """

    xseq = pycrfsuite.ItemSequence()
    for x in X:
        item = pycrfsuite.Item()
        for f in x['F']:
            if isinstance(f, str):
                item.append(pycrfsuite.Attribute(escape(f)))
            else:
                item.append(pycrfsuite.Attribute(escape(f[0]), f[1]))
        xseq.append(item)
    return xseq
...
-t option (CRFsuite Python module must be installed)."""
    ...
    # The fields of input: ('w', 'pos', 'y) by default: `word postag label` for each file line
